...
def create_device(address, busnum):
    ...
    # CardosoTech_GPIO.I2C module can be imported.  Also inject a mock SMBus
...
def safe_import_i2c():
    # Mock the smbus module and inject it into the global namespace so the
    # CardosoTech_GPIO.I2C module can be imported.  The imported I2C module is
...
class TestGetDefaultBus(unittest.TestCase):
    @patch('CardosoTech_GPIO.Platform.pi_revision', Mock(return_value=1))
    ...
    @patch('CardosoTech_GPIO.Platform.pi_revision', Mock(return_value=2))
    @patch('CardosoTech_GPIO.Platform.platform_detect', Mock(return_value=Platform.RASPBERRY_PI))
